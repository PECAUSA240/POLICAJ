...
def registro_usuarios():
    ...
    id_usuario = simular_input("Ingrese su número de identificación (solo números): ")
    ...
    contrasena = simular_input("Ingrese una contraseña (solo números): ")
...
def inicio_sesion(usuarios):
    ...
    while True:
        id_usuario = simular_input("Ingrese su número de identificación: ")
        if not validar_numeros(id_usuario, 10):
            print("Error: Solo se permiten números de 10 dígitos. Intente nuevamente.")
            continue

        numero_cuenta = simular_input("Ingrese su número de cuenta: ")
        if not validar_numeros(numero_cuenta, 6):
            print("Error: Solo se permiten números de 6 dígitos. Intente nuevamente.")
            continue

        contrasena = simular_input("Ingrese su contraseña: ")
        if not validar_numeros(contrasena, 6):
            print("Error: Solo se permiten números de 6 dígitos. Intente nuevamente.")
            continue

        for usuario in usuarios:
            if (usuario[0] == id_usuario and
                str(usuario[1]) == numero_cuenta and
...
def simular_input(prompt):
    """Simula la entrada para entornos no interactivos."""
    print(prompt)
    return ""  # Reemplazar con valores predeterminados o simulados
...
def menu_principal():
    ...
    usuarios.append((id_usuario, numero_cuenta, contrasena))
    if not inicio_sesion(usuarios):
        return
    ...
    while True:
        ...
        opcion = simular_input("Seleccione una opción: ")
        if opcion == "1":
            cajero.consultar_saldo()
        elif opcion == "2":
            try:
                monto = float(simular_input("Ingrese el monto a depositar: "))
                cajero.depositar_dinero(monto)
            except ValueError:
                print("Error: Por favor, ingrese un monto válido.")
        elif opcion == "3":
            try:
                monto = float(simular_input("Ingrese el monto a retirar: "))
                cajero.retirar_dinero(monto)
            except ValueError:
                print("Error: Por favor, ingrese un monto válido.")
        elif opcion == "4":
            cuenta_destino = simular_input("Ingrese el número de cuenta destino: ")
            if not validar_numeros(cuenta_destino, 6):
                print("Error: Número de cuenta no válido. Debe ingresar 6 dígitos.")
                continue
            try:
                monto = float(simular_input("Ingrese el monto a transferir: "))
